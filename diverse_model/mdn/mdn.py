...
flags.DEFINE_integer("num_test_batches", 100, "test batch size")
...
def cnn_feedforward(lf, input_tensor, bn_is_training, keep_prob, reuse=False):
	...
	conv1_norm = lf.batch_norm_aiuiuc_wrapper(conv1, bn_is_training, 'BN1', reuse_vars=reuse)

	conv1_1 = tf.nn.relu(lf.conv2d(conv1_norm, W_conv1_1, stride=1))
	conv1_1_norm = lf.batch_norm_aiuiuc_wrapper(conv1_1, bn_is_training, 'BN1_1', reuse_vars=reuse)
	
	conv1_2 = tf.nn.relu(lf.conv2d(conv1_1_norm, W_conv1_2, stride=1))
	conv1_2_norm = lf.batch_norm_aiuiuc_wrapper(conv1_2, bn_is_training, 'BN1_2', reuse_vars=reuse)

	conv1_3 = tf.nn.relu(lf.conv2d(conv1_2_norm, W_conv1_3, stride=2))
	conv1_3_norm = lf.batch_norm_aiuiuc_wrapper(conv1_3, bn_is_training,'BN1_3', reuse_vars=reuse)

	conv2 = tf.nn.relu(lf.conv2d(conv1_3_norm, W_conv2, stride=1))
	conv2_norm = lf.batch_norm_aiuiuc_wrapper(conv2, bn_is_training, 'BN2', reuse_vars=reuse)
	
	dropout1 = tf.nn.dropout(conv2_norm, rate=1 - (keep_prob))
	flatten1 = tf.reshape(dropout1, [-1, 14*14*128])
	fc1 = tf.tanh(tf.matmul(flatten1, W_fc1)+b_fc1)

	dropout2 = tf.nn.dropout(fc1, rate=1 - (keep_prob))
...
def compute_gmm_loss(gt_tensor, op_tensor_activ, summ=False):
	...
	op_norm_dist = tf.reshape(tf.compat.v1.div((.5*tf.reduce_sum(input_tensor=tf.square(gt_tensor_flat-op_tensor_mu_flat),axis=1)), op_tensor_sigma_flat), [FLAGS.batch_size, FLAGS.nmix])
	op_norm_dist_min = tf.reduce_min(input_tensor=op_norm_dist, axis=1)
	op_norm_dist_minind = tf.cast(tf.argmin(input=op_norm_dist, axis=1), dtype=tf.int32)
	...
	if(summ == True):
		gmm_loss = tf.reduce_mean(input_tensor=-tf.math.log(op_pi_min+1e-30) + op_norm_dist_min, axis=0)
	else:
		gmm_loss = tf.reduce_mean(input_tensor=op_norm_dist_min, axis=0)
	
	if(summ == True):
		tf.compat.v1.summary.scalar('gmm_loss', gmm_loss)
		tf.compat.v1.summary.scalar('op_norm_dist_min', tf.reduce_min(input_tensor=op_norm_dist))
		tf.compat.v1.summary.scalar('op_norm_dist_max', tf.reduce_max(input_tensor=op_norm_dist))
		tf.compat.v1.summary.scalar('op_pi_min', tf.reduce_mean(input_tensor=op_pi_min))
...
def main():
	...
	#Inputs
	lf = layer_factory()
	input_tensor = tf.compat.v1.placeholder(tf.float32, [FLAGS.batch_size, FLAGS.feats_height, FLAGS.feats_width, FLAGS.feats_nch])
	output_gt_tensor = tf.compat.v1.placeholder(tf.float32, [FLAGS.batch_size, FLAGS.hidden_size])
	is_training = tf.compat.v1.placeholder(tf.bool)
	keep_prob = tf.compat.v1.placeholder(tf.float32)

	#Inference
	with tf.compat.v1.variable_scope('Inference', reuse=False):
	...
	#Standard steps		
	check_nan_op = tf.compat.v1.add_check_numerics_ops()
	...
	train_writer = tf.compat.v1.summary.FileWriter(os.path.join(FLAGS.in_lvdir, 'logs_mdn'), sess.graph)
