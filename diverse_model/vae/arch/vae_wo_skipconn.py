...
class vae_wo_skipconn:
	...
	def inputs(self):
		...
		is_training_dec = tf.compat.v1.placeholder(tf.bool)
    ...
    is_regression=True):
		...
		recon_loss_chi = tf.reduce_mean(input_tensor=tf.sqrt(tf.reduce_sum( \
    ...
    #Load Principle components
		...
		loss_topk_pc = tf.reduce_mean(input_tensor=tf.reduce_sum(\
      input_tensor=tf.multiply(tf.square(projmat_op-projmat_target), weightmat), axis=1), axis=0)

		res_op = op_tensor
		res_target = target_tensor
		for npc in range(self.flags.pc_comp):
			pcvec_curr = tf.tile(tf.reshape(tf.transpose(a=pcvec[:, npc]), [1, -1]), \
		...
		res_error = tf.reduce_sum(input_tensor=tf.square(res_op-res_target), axis=1)
		res_error_weight = tf.tile(tf.reshape(pcvar[self.flags.pc_comp-1], [1, 1]), [self.flags.batch_size, 1])
		loss_res_pc = tf.reduce_mean(input_tensor=tf.multiply(\
      tf.reshape(res_error, [self.flags.batch_size, 1]), res_error_weight))
		...
		tf.compat.v1.summary.scalar('recon_loss_chi', recon_loss_chi)
	...
	def __decoder(self, scope, bn_is_training, inp_greylevel, z1_sample, reuse=False):
		...
		deconv1_upsamp = tf.image.resize(input_concat2d, [4, 4])
	...
	def __get_gradients(self, in_tensor2d, shape=None):
		if(shape is not None):
			in_tensor = tf.image.resize(in_tensor2d, [shape[0], shape[1]])	
		else:
