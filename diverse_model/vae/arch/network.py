...
class network:
	...
	def __build_graph(self):
		...
		#standard steps
		self.check_nan_op = tf.compat.v1.add_check_numerics_ops()
	...
	def train_vae(self, chkptdir, is_train=True):
		...
		if(is_train == True):
			...
			self.train_writer = tf.compat.v1.summary.FileWriter(os.path.join(self.flags.out_dir, 'logs'), sess.graph)
