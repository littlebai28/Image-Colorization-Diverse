...
class vae_skipconn:
	...
	def inputs(self):
		inp_img = tf.compat.v1.placeholder(tf.float32, [self.flags.batch_size, \
			self.nch * self.flags.img_height * self.flags.img_width])
		inp_greylevel = tf.compat.v1.placeholder(tf.float32, [self.flags.batch_size, \
			self.flags.img_height * self.flags.img_width])
		inp_latent = tf.compat.v1.placeholder(tf.float32, [self.flags.batch_size, \
			self.flags.hidden_size])
		is_training = tf.compat.v1.placeholder(tf.bool)
		keep_prob = tf.compat.v1.placeholder(tf.float32)
		kl_weight = tf.compat.v1.placeholder(tf.float32)
		lossweights = tf.compat.v1.placeholder(tf.float32, [self.flags.batch_size, \
			self.nch * self.flags.img_height * self.flags.img_width])
	...
	def inference(self, inp_img, inp_greylevel, inp_latent, is_training, keep_prob):
		...
		with tf.compat.v1.variable_scope('Inference', reuse=False) as sc:
			...
			epsilon_train = tf.random.truncated_normal([self.flags.batch_size, self.flags.hidden_size])
		...
		with tf.compat.v1.variable_scope('Inference', reuse=True) as sc:
			gfeat32, gfeat16, gfeat8, gfeat4 = \
				self.__cond_encoder(sc, inp_greylevel, is_training, keep_prob, \
					in_nch=1, reuse=True)
			...
			if(self.condinference_flag == False):
				...
				epsilon_test = tf.random.truncated_normal([self.flags.batch_size, self.flags.hidden_size])
	...
	def loss(self, target_tensor, op_tensor, mean, stddev, kl_weight, lossweights, epsilon=1e-6):
		...
		kl_loss = tf.reduce_sum(input_tensor=0.5 * (tf.square(mean) + tf.square(stddev) \
					- tf.math.log(tf.maximum(tf.square(stddev), epsilon)) - 1.0))

		recon_loss = tf.reduce_mean(input_tensor=tf.sqrt(tf.reduce_sum( \
			     input_tensor=lossweights*tf.square(target_tensor-op_tensor), axis=1)), axis=0)

		recon_loss_l2 = tf.reduce_mean(input_tensor=tf.sqrt(tf.reduce_sum( \
			     input_tensor=tf.square(target_tensor-op_tensor), axis=1)), axis=0)
		...
		if(self.nch == 2):
			...
			grad_loss = tf.reduce_mean(input_tensor=tf.sqrt(tf.reduce_sum(input_tensor=dv+dh,axis=[1,2,3])))
			recon_loss = recon_loss + (1e-3)*grad_loss

		loss = kl_weight*kl_loss + recon_loss
		tf.compat.v1.summary.scalar('kl_loss', kl_loss)
		tf.compat.v1.summary.scalar('grad_loss', grad_loss)
		tf.compat.v1.summary.scalar('recon_loss', recon_loss)
		tf.compat.v1.summary.scalar('recon_loss_l2', recon_loss_l2)
		tf.compat.v1.summary.scalar('loss', loss)
		return loss
	
	#Takes loss and returns GD train step
	def optimize(self, loss, epsilon):
		train_step = tf.compat.v1.train.AdamOptimizer(self.flags.lr_vae, epsilon=epsilon).minimize(loss) 
		return train_step
	...
	def __encoder(self, scope, input_tensor, bn_is_training, keep_prob, in_nch=2, reuse=False):
		...
		dropout1 = tf.nn.dropout(conv4_norm, rate=1 - (keep_prob))
	...
	def __decoder(self, scope, gfeat32, gfeat16, gfeat8, gfeat4, bn_is_training, inp_greylevel,\
		...
		inp_greylevel2d = tf.reshape(inp_greylevel, [self.flags.batch_size, self.flags.img_height, self.flags.img_width, 1])
		input2d = tf.reshape(z1_sample, [self.flags.batch_size, 1, 1, self.flags.hidden_size])
		deconv1_upsamp = tf.image.resize(input2d, [4, 4])
		...
		deconv1_norm = lf.batch_norm_aiuiuc_wrapper(deconv1, bn_is_training, 'BN_deconv1', reuse_vars=reuse)
		
		deconv2_upsamp = tf.image.resize(deconv1_norm, [8, 8])
		...
		deconv2_norm = lf.batch_norm_aiuiuc_wrapper(deconv2, bn_is_training,'BN_deconv2', reuse_vars=reuse)

		deconv3_upsamp = tf.image.resize(deconv2_norm, [16, 16])
		...
		deconv3_norm = lf.batch_norm_aiuiuc_wrapper(deconv3, bn_is_training,'BN_deconv3', reuse_vars=reuse)

		deconv4_upsamp = tf.image.resize(deconv3_norm, [32, 32])
		...
		deconv4_norm = lf.batch_norm_aiuiuc_wrapper(deconv4, bn_is_training, 'BN_deconv4', reuse_vars=reuse)

		deconv5_upsamp = tf.image.resize(deconv4_norm, [64, 64])
		deconv5 = lf.conv2d(deconv5_upsamp, W_deconv5, stride=1) + b_deconv5
		deconv5_norm = lf.batch_norm_aiuiuc_wrapper(deconv5, bn_is_training,'BN_deconv5', reuse_vars=reuse)
		
		decoded_ch = tf.reshape(tf.tanh(deconv5_norm),[self.flags.batch_size, self.flags.img_height*self.flags.img_width*self.nch])
		return decoded_ch
